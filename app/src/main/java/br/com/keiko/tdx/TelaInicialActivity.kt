...
class TelaInicialActivity : DebugActivity(), NavigationView.OnNavigationItemSelectedListener {
    ...
    private var grupo = listOf<Grupo>()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // configurar cardview
        recyclerGrupo?.layoutManager = LinearLayoutManager(context)
        recyclerGrupo?.itemAnimator = DefaultItemAnimator()
    ...
    fun taskGrupo() {
        ...
        // Criar a Thread
        Thread {
            // Código para procurar as grupo
            // que será executado em segundo plano / Thread separada
            this.grupo = GrupoService.getGrupo(context)
            runOnUiThread {
                // Código para atualizar a UI com a lista de grupo
                recyclerGrupo?.adapter = GrupoAdapter(this.grupo) { onClickGrupo(it) }
                // enviar notificação
                enviaNotificacao(this.grupo.get(0))
    ...
    fun enviaNotificacao(grupo: Grupo) {
        // Intent para abrir tela quando clicar na notificação
        val intent = Intent(this, GrupoActivity::class.java)
        // parâmetros extras
        intent.putExtra("grupo", grupo)
        // Disparar notificação
        NotificationUtil.create(this, 1, intent, "LMSApp", "Você tem nova atividade na ${grupo.nome}")
    }

    // tratamento do evento de clicar em uma grupo
    fun onClickGrupo(grupo: Grupo) {
        Toast.makeText(context, "Clicou grupo ${grupo.nome}", Toast.LENGTH_SHORT).show()
    ...
    override fun onNavigationItemSelected(item: MenuItem): Boolean {
        when (item.itemId) {
            ...
            R.id.nav_candidatos -> {
                Toast.makeText(this, "Informações para Candidatos - Em construção", Toast.LENGTH_SHORT).show()
            }

            R.id.nav_empresas -> {
                Toast.makeText(this, "Informações para Empresas - Em construção", Toast.LENGTH_SHORT).show()
            }
    ...
    override fun onOptionsItemSelected(item: MenuItem?): Boolean {
        ...
        if  (id == R.id.action_buscar) {
            ...
            val intent = Intent(context, GrupoCadastroActivity::class.java)
    ...
    // esperar o retorno do cadastro da grupo
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        if (requestCode == REQUEST_CADASTRO || requestCode == REQUEST_REMOVE ) {
            // atualizar lista de grupo
            taskGrupo()
        }
    }
